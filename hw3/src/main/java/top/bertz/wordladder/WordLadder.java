...
public class WordLadder {
    ...
    public ArrayList<Integer> search(int s_idx, String t) {
        ...
        ArrayList<Integer> reversed_result = new ArrayList<Integer>();
        ...
        for (int i = reversed_result.size() - 1; i >= 0; --i)
            ret.add(reversed_result.get(i));
        return ret;
    }

    public ArrayList<String> BFS(String startword, String endword) {
        int s_idx = this.dict.indexOf(startword);
        ArrayList<String> result_list = new ArrayList<String>();
        ArrayList<Integer> result = this.search(s_idx, endword);
        if (result == null) {
            return null;
        }
        for (int i = 0; i < result.size(); ++i) {
            result_list.add(this.dict.get(result.get(i)));
        }
        return result_list;
